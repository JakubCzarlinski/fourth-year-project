...
def attack_forward(
  ...
  latents = torch.randn(
      latents_shape, device=self.device, dtype=text_embeddings.dtype
  )
...
for file_iteration in file_iteration_names:
  ...
  init_image = Image.open(f'../dataset/{file_iteration}.png'
                         ).convert('RGB').resize((400, 400))
  mask_image = Image.open(f'../dataset/{file_iteration}_masked.png'
                         ).convert('RGB')
