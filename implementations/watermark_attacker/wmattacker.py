...
class DiffWMAttacker(WMAttacker):
  ...
  def __init__(
      self, pipe: ReSDPipeline, batch_size=20, noise_step=60, captions={}
  ):
    self.pipe = pipe
  ...
  def attack(
    ...
    for img_path, out_path, prompt in tqdm(
        zip(image_paths, out_paths, prompts)
    ):
      ...
      img = (img - 0.5) * 2
      img = torch.tensor(
          img, dtype=torch.float16, device=self.device
      ).permute(2, 0, 1).unsqueeze(0)
      latents = self.pipe.vae.encode(img).latent_dist
      latents = latents.sample(generator) * self.pipe.vae.config.scaling_factor
      noise = torch.randn(
          [1, 16, img.shape[-2] // 8, img.shape[-1] // 8], device=self.device
      )
      if return_dist:
        return self.pipe.scheduler.scale_noise(
            sample=latents,
            noise=noise,
            timestep=timestep,
            return_dist=True,
        )
      latents = self.pipe.scheduler.scale_noise(
          sample=latents,
          noise=noise,
          timestep=timestep,
      ).type(torch.half)
      ...
      if len(latents_buf) == self.BATCH_SIZE:
        self.batched_attack(latents_buf, prompts_buf, outs_buf, generator)
        latents_buf = []
        prompts_buf = []
        outs_buf = []
      if return_latents:
  ...
  def batched_attack(self, latents_buf, prompts_buf, outs_buf, generator):
