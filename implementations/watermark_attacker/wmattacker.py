...
class DiffWMAttacker(WMAttacker):
    def __init__(self, pipe: ReSDPipeline, batch_size=20, noise_step=60, captions={}):
        self.pipe = pipe
    ...
    def attack(self, image_paths, out_paths, return_latents=False, return_dist=False):
        with torch.no_grad():
            ...
            def batched_attack(latents_buf, prompts_buf, outs_buf):
                ...
                images = self.pipe.__call__(prompts_buf,
            ...
            for (img_path, out_path), prompt in tqdm(zip(zip(image_paths, out_paths), prompts)):
                ...
                noise = torch.randn([1, 16, img.shape[-2] // 8, img.shape[-1] // 8], device=self.device)
                self.pipe.scheduler: FlowMatchEulerDiscreteScheduler = self.pipe.scheduler
                # print(f"latents shape: {latents.shape}, noise shape: {noise.shape}, timestep: {timestep}")
                if return_dist:
                    return self.pipe.scheduler.scale_noise(sample=latents, noise=noise, timestep=timestep, return_dist=True)
                latents = self.pipe.scheduler.scale_noise(sample=latents, noise=noise, timestep=timestep).type(torch.half)
