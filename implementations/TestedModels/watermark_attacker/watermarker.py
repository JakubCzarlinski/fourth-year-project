...
class StableSignatureWatermarker(Watermarker):
  ...
  def __init__(
    ...
    self.key = key
    self.msg_extractor = msg_extractor = torch.jit.load(msg_extractor
                                                       ).to("cuda")
  ...
  def encode(self, img_path, output_dir, prompt=''):
    command = [
        'python',
        os.path.join(self.stable_diffusion_root_path, f'scripts/{self.script}'),
        '--prompt',
        prompt,
        '--ckpt',
        os.path.join(
            self.stable_diffusion_root_path,
            'checkpoints/v2-1_512-ema-pruned.ckpt'
        ),
        '--config',
        os.path.join(
            self.stable_diffusion_root_path,
