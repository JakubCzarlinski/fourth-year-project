...
def optimize_prompt_loop(
    model, tokenizer, token_embedding, all_target_features, args, device
):
  opt_iters = args.iter
  lr = args.lr
  ...
  input_optimizer = torch.optim.AdamW(
      [prompt_embeds], lr=lr, weight_decay=weight_decay
  )
  ...
  for step in range(opt_iters):
    ...
    with torch.no_grad():
      ...
      logits_per_image, _ = model.forward_text_embedding(
          padded_embeds, dummy_ids, universal_target_features
      )
    ...
    logits_per_image, _ = model.forward_text_embedding(
        padded_embeds, dummy_ids, target_features
    )
    ...
    if print_step is not None and (
        step % print_step == 0 or step == opt_iters - 1
    ):
...
def optimize_prompt(
    model, preprocess, args, device, target_images=None, target_prompts=None
):
...
def measure_similarity(
    orig_images, images, ref_model, ref_clip_preprocess, device
):
  with torch.no_grad():
