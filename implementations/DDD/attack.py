...
for file_iteration in file_iteration_names:
  ...
  token_embedding = (
      pipe_inpaint.text_encoder.text_model.embeddings.token_embedding
  )
  ...
  orig_images = Image.open(f'./dataset/{testimg_filename}.png'
                          ).convert('RGB').resize((512, 512))
  mask_image = Image.open(f'./dataset/{testimg_filename}_masked.png'
                         ).convert('RGB').resize((512, 512))
  mask_image = ImageOps.invert(mask_image).resize((512, 512))
  # mask_image = ImageOps.invert(mask_image).resize((512,512))

  # mask_image.show()
  cur_mask, cur_masked_image, init_image = utils.utils.prepare_mask_and_masked2(
      orig_images, mask_image, no_mask=False, inverted=True
  )
  inv_cur_mask, _, _ = utils.utils.prepare_mask_and_masked2(
  ...
  with torch.no_grad():
    ...
    all_latents = pipe_inpaint.vae.encode(curr_images.to(weight_dtype)
                                         ).latent_dist.sample()
    all_latents = all_latents * 0.18215
    masked_image_latents = pipe_inpaint.vae.encode(
        cur_masked_image
  ...
  input_optimizer = torch.optim.AdamW(
      [prompt_embeds], lr=args.lr, weight_decay=args.weight_decay
  )
  ...
  for step in range(args.opt_iters):
    ...
    model_pred = pipe_inpaint.unet(
        input_latent, timesteps, encoder_hidden_states=text_embeddings
    ).sample
    inverted_mask = mask
  ...
  init_image = Image.open(f'./dataset/{testimg_filename}.png'
                         ).convert('RGB').resize((512, 512))
  mask_image = Image.open(f'./dataset/{testimg_filename}_masked.png'
                         ).convert('RGB')
  mask_image = ImageOps.invert(mask_image).resize((512, 512))
  ...
  latents_shape = (
      1, pipe_inpaint.vae.config.latent_channels, 512 // 8, 512 // 8
  )
  noise = torch.randn(
      latents_shape, device=pipe_inpaint.device, dtype=text_embeddings.dtype
  )
  ...
  for n, m in pipe_inpaint.unet.named_modules():
    ...
    if (n.endswith('attn1') and (modes[mode] in n)):  #and "down" in n:
      m.set_processor(ddd.MyCrossAttnProcessor(attn_controller, n))
      module_count += 1
  ...
  for j in range(n_samples):
    with torch.no_grad():
      ...
      latents = torch.randn(
          latents_shape, device=pipe_inpaint.device, dtype=uncond_emb.dtype
      )
      ...
      timesteps_tensor = pipe_inpaint.scheduler.timesteps.to(
          pipe_inpaint.device
      )
  ...
  result, last_image, total_losses = ddd.disrupt(
      cur_mask,
      cur_masked_image,
  ...
  adv_image.save(f'./dataset_adv/{testimg_filename}_adv.png')
