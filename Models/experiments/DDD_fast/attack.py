...
for testing_filename in file_iteration_names:
  ...
  orig_images = Image.open(f'./images/{testing_filename}.png'
                          ).convert('RGB').resize(size_2d)
  mask_image_orig = Image.open(f'./images/{testing_filename}_masked.png'
                              ).convert('RGB').resize(size_2d)
  ...
  with torch.no_grad():
    ...
    gt_embeddings = utils.get_text_embedding(pipe_inpaint, testing_filename)
  ...
  prefix_filename = "./images/" + testing_filename
  ...
  infer_dict = dict()
  infer_dict["prompt"] = utils.load_prompt(f"prompts/{testing_filename}.txt")[0]
  ...
  adv_image.save(f'./adversarial/{testing_filename}_adv.png')

  prompts = utils.load_prompt(f"prompts/{testing_filename}.txt")
  print(prompts)
