...
import torch
from diffusers.pipelines.stable_diffusion_3 import pipeline_output
...
class ReSDPipeline(pipeline_stable_diffusion_3.StableDiffusion3Pipeline):
  ...
  def __call__(
    ...
    Args:
        ...
        latents (`torch.FloatTensor`, *optional*):
            ...
            to be used as inputs for image generation. Can be used to tweak the
            same generation with different prompts. If not provided, a latents
            tensor will ge generated by sampling using the supplied random
            `generator`.
        output_type (`str`, *optional*, defaults to `"pil"`):
            The output format of the generate image. Choose between
            [PIL](https://pillow.readthedocs.io/en/stable/): `PIL.Image.Image`
            or `np.array`.
        ...
        callback_steps (`int`, *optional*, defaults to 1):
            The frequency at which the `callback` function will be called. If
            not specified, the callback will be called at every step.

    Returns:
        [`~pipelines.stable_diffusion.StableDiffusionPipelineOutput`] or
        `tuple`: [`~pipelines.stable_diffusion.StableDiffusionPipelineOutput`]
        if `return_dict` is True, otherwise a `tuple. When returning a tuple,
        the first element is a list with the generated images, and the second
        element is a list of `bool`s denoting whether the corresponding
        generated image likely represents "not-safe-for-work" (nsfw) content,
    ...
    if do_classifier_free_guidance:
      ...
      if prompt2 is not None:
        prompt_embeds2 = torch.cat(
            [negative_prompt_embeds2, prompt_embeds2], dim=0
        )
        pooled_prompt_embeds2 = torch.cat(
            [negative_pooled_prompt_embeds2, pooled_prompt_embeds2], dim=0
        )

    # 4. Prepare timesteps
    timesteps, num_inference_steps = pipeline_stable_diffusion_3.retrieve_timesteps(
        self.scheduler,
        num_inference_steps,
        device,
    )
    ...
    return pipeline_output.StableDiffusion3PipelineOutput(images=image)
